                ...
                while (true)
                {
                    ...
                    if (inboundBuffer.bufferLength != 0)
                    {
                        if (sendHandle.data != IntPtr.Zero && inboundBuffer.bufferWithHeaders == (byte*)sendHandle.data + initialHeaderSize + 1)
                        {
                            ...
                            {
                                UnityEngine.Debug.LogWarning(FixedString.Format("[NetworkPipeline] CompleteSend failed with the following error code: {0}", retval));
                            }
                            sendHandle = default;
                        }
                        else
                        {
                            ...
                            if (driver.BeginSend(connection, out var writer) == 0)
                            {
                                ...
                                {
                                    UnityEngine.Debug.Log(FixedString.Format("[NetworkPipeline] An error occurred during EndSend. ErrorCode: {0}", retval));
                                }
                            }
                        }
                    }
