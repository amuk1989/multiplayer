                ...
                while (!success)
                {
                    ...
                    if (driver.BeginSend(pipelineToUse, connectionId, out var dataStream, targetSnapshotSize) == 0)
                    {
                        ...
                        if (success)
                        {
                            ...
                            {
                                netDebug.LogWarning($"[GhostSendSystem] target of message: {idx}, snapshot size {targetSnapshotSize}");
                                netDebug.LogWarning(FixedString.Format("[GhostSendSystem] An error occurred during EndSend. ErrorCode: {0}", result));
                            }
                            netDebug.LogWarning($"[GhostSendSystem] target message without error: {idx}, snapshot size {targetSnapshotSize}");
                        }
                        else
